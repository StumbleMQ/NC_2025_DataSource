...
###
getwd()
# setwd("~/Downloads/NC_2025_RNAseq_DataSource/MainFigure/Figure_1d")
...
gseaplot2(gsea, geneSetID = "hsa05321",
          ...
          ES_geom = "line")
#######################################################################################
### Alternatively arrange the genes list by score 
...
### manually construct score column using excel Or using the following code to build a score ranking file
exp.fc.id.score <- exp.fc.id.score[order(exp.fc.id.score$pvalue, decreasing = F),]
head(exp.fc.id.score, 5)

###  sign in a relevant vale into "0" in the first four genes name because log10(0) gives a N/A value
